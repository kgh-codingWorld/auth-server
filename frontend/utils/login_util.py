...
from frontend.configs.base_url_config import BASE_URL # FastAPI 서버의 기본 URL 설정

# 로그인 요청 함수 (Gradio → FastAPI)
def login(username, password):
    try:
        # FastAPI의 '/login' 엔드포인트로 POST 요청을 보냄
        response = requests.post(f"{BASE_URL}/auth/login", json={"username": username, "password": password}, # JSON 형태로 데이터 전송
        ...
        if response.status_code == 200:
            ...
            return "", f"로그인 실패: {response.json().get('detail', 'Unknown Error')}"
